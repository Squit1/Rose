...
class ModelConfig:
    ...
    save_dir: str = f"{HOME}/Noesis_v2"
    output_dir: Optional[str] = f"{HOME}/model_output"
    
    data_path: str = f"{HOME}/processed_data/all_packed.pt"
...
class GQAAttention(nn.Module):
    """
    Grouped Query Attention (GQA) — LLaMA 3 / Mistral tarzı.
    num_kv_heads < num_heads: KV cache belleği düşer, hız artar.
    """
    def __init__(self, d_model, num_heads, num_kv_heads=None, dropout=0.0,
                 use_bitlinear=False, quantize_training=False):
        super().__init__()
        ...
        create_linear = lambda i, o: (
            BitLinear(i, o, bias=False, quantize_training=quantize_training)
            if use_bitlinear else nn.Linear(i, o, bias=False)
        )

        self.q_proj = create_linear(d_model, num_heads * self.head_dim)
        self.k_proj = create_linear(d_model, self.num_kv_heads * self.head_dim)
        self.v_proj = create_linear(d_model, self.num_kv_heads * self.head_dim)
        self.out_proj = create_linear(num_heads * self.head_dim, d_model)
    ...
    def forward(self, x, mask=None):
        ...
        # KV head'leri num_heads'e genişlet (repeat_interleave)
        ...
        try:
            ...
            # Fallback: manuel attention
            attn = (q @ k.transpose(-2, -1)) * self.scale
            causal = torch.ones(T, T, device=x.device).tril().bool()
...
class GQABlock(nn.Module):
    ...
    def __init__(self, config, use_bitlinear=False, quantize_training=False):
        ...
        self.ffn1 = create_linear(config.d_model, config.dim_feedforward)
        self.ffn2 = create_linear(config.dim_feedforward, config.d_model)
        self.drop = nn.Dropout(config.dropout)

    def forward(self, x, mask=None):
        x = x + self.attn(self.norm1(x), mask=mask)
        x = x + self.drop(self.ffn2(F.gelu(self.ffn1(self.norm2(x)))))
        return x
...
class BitLinear(nn.Linear):
    ...
    def quantize_weight(self, w):
        """Ağırlık kuantizasyonu: float -> {-1, 0, 1} (group-wise)"""
        ...
        if in_features % gs != 0:
            ...
            return w_q, scale
        # Group-wise absmax scale
        w_grouped = w.view(out_features, -1, gs)
        scale = w_grouped.abs().mean(dim=-1, keepdim=True).clamp(min=self.eps)
        w_normalized = w_grouped / scale
        w_q = torch.clamp(torch.round(w_normalized), -1, 1)
        w_q = w_q + (w_normalized - w_q).detach()
        w_q = w_q.view(out_features, in_features)
        scale_scalar = scale.mean()
        return w_q, scale_scalar
    def forward(self, x):
        ...
        if not self.quantize_training:
            ...
            return output * scale  # weight_scale kaldırıldı: BitNet b1.58 sadece absmax scale kullanır
...
def selective_scan_matrix_mode(u, delta, A, B, C, D):
    ...
    # deltaA: (b, l, d_in, n)
    # deltaA: log-space'de tut, exp almadan cumsum yap
    log_deltaA = torch.einsum('bld,dn->bldn', delta, A)  # log(deltaA) = delta * A
    deltaB_u = torch.einsum('bld,bln,bld->bldn', delta, B, u)
    # Stable Parallel Scan - log-space cumsum
    cum_log_A = torch.cumsum(log_deltaA, dim=1).clamp(min=-20.0, max=5.0)
    cum_A = torch.exp(cum_log_A)
    cum_B = torch.cumsum(deltaB_u / (cum_A.detach() + 1e-8), dim=1)
    x = cum_B * cum_A
...
class HybridBlock(nn.Module):
    ...
    def __init__(self, config, use_mamba=True, use_bitlinear=True, 
        ...
        if use_mamba:
            ...
            # GQA Attention bloğu (LLaMA 3 / Mistral tarzı)
            self.main = GQAAttention(
                d_model=config.d_model,
                num_heads=config.nhead,
                num_kv_heads=max(1, config.nhead // 4),
                dropout=config.attention_dropout,
    ...
    def forward(self, x, mask=None, **kwargs):
        ...
        if isinstance(self.main, GQAAttention):
            x = x + self.main(self.norm1(x), mask=mask)
        else:
...
class UltimateTransformerModel(nn.Module):
    ...
    def __init__(self, c):  # c: ModelConfig
        ...
        # 4. Hybrid Decoder - 1:7 Attention:Mamba oranı (Jamba mimarisi)
        decoder_layers = []
        for layer_idx in range(c.num_decoder_layers):
            use_mamba = (layer_idx + 1) % 4 != 0
            layer = HybridBlock(
                c,
                use_mamba=use_mamba,
                use_bitlinear=getattr(c, "quantize_training", False),
                quantize_training=getattr(c, "quantize_training", False)
            )
            decoder_layers.append(layer)
        self.decoder = nn.ModuleList(decoder_layers)
        
        self.decoder_norm = RMSNorm(c.d_model)
    ...
    def forward(self, input_ids: torch.Tensor, attention_mask: Optional[torch.Tensor] = None, labels: Optional[torch.Tensor] = None, **kwargs) -> Any:
        ...
        if labels is not None:
            # ⚠️ UYARI: Training loop labels=None geçiyor, loss WeightedCrossEntropyLoss
            # (criterion) içinde hesaplanıyor ve orada shift uygulanıyor.
            # Buraya labels geçerseniz ÇİFT SHIFT olur — lm_labels'ı direkt
            # criterion'a verin, forward'a labels geçmeyin.
...
class MultiTaskEvaluator:
    ...
    def evaluate(self, model, eval_loader, criterion, device):
        ...
        # Autocast context
        device_type = 'cuda' if device.type == 'cuda' else 'cpu'  # XLA için nullcontext kullanılacak
...
class autocast_context:
    """Mixed precision autocast için context manager (TPU/GPU/CPU uyumlu)"""
    def __init__(self, device, dtype, enabled):
        self.device = device
        self.dtype = dtype
    ...
    def __enter__(self):
        ...
        if self.device.type == 'cuda':
            self._ctx = torch.autocast(device_type='cuda', dtype=self.dtype)
            self._ctx.__enter__()
        elif self.device.type == 'cpu':
            try:
                self._ctx = torch.autocast(device_type='cpu', dtype=torch.bfloat16)
                self._ctx.__enter__()
            except RuntimeError:
                pass
        # xla: no-op, BF16 XLA compiler seviyesinde uygulanır
        return self
...
class AdvancedUltimateTrainer:
    ...
    def train(self, train_dataset: Dataset, eval_dataset: Optional[Dataset] = None,
        ...
        _opt_ref = {"optimizer": None, "scheduler": None}
        ...
        def signal_handler(signum, frame):
            if self.is_master():
                ...
                if _opt_ref["optimizer"] is None:
                    self.logger.warning("⚠️ Optimizer henüz hazır değil, checkpoint atlanıyor.")
                    sys.exit(1)
                checkpoint_manager.save_checkpoint(
                    "signal_interrupted.pt", state,
                    _opt_ref["optimizer"], _opt_ref["scheduler"], {}, force_full=True
                )
        ...
        completed_epochs = resume_from_ckpt_data.get("epoch", 0) + 1 if resume_from_ckpt_data else 0
        remaining_epochs = max(num_epochs - completed_epochs, 1)
        ...
        _opt_ref["optimizer"] = optimizer
        _opt_ref["scheduler"] = scheduler
            ...
            class PackedTPUDataset(Dataset):
                def __init__(self, data_tensor, labels=None):
                    if isinstance(data_tensor, dict):
                        self.data = data_tensor['input_ids']
                        self.labels = data_tensor.get('labels', None)
                    else:
                        self.data = data_tensor
                        self.labels = labels
                ...
                def __getitem__(self, idx):
                    input_ids = self.data[idx]
                    lm_labels = self.labels[idx] if self.labels is not None else input_ids.clone()
                    attention_mask = torch.ones_like(input_ids)
                    return {
                        'input_ids': input_ids,
                        'attention_mask': attention_mask,
    ...
    try:
        if config.is_tpu and XLA_AVAILABLE:
            ...
            try:
                ...
                elif "operation not permitted" in error_msg.lower() or "bad statusor" in error_msg.lower():
                    initial_logger.warning("="*80)
                    initial_logger.warning("⚠️ TPU ERİŞİM HATASI — CPU moduna geçiliyor...")
                    initial_logger.warning(f"   └─ Hata: {error_msg[:150]}")
                    initial_logger.warning("="*80)
                    config.is_tpu = False
                    config.is_tpu_v5e = False
                    config.world_size = 1
                    _mp_fn(0, (config, args_cli))
                else:
